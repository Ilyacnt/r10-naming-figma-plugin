...
figma.ui.onmessage = (msg) => {
    if (msg.type === EFigmaMessageType.CANCEL) {
        ...
        const renameNode = (node: SceneNode, number: String | Number, isTemplateNaming: Boolean) => {
            if (!isTemplateNaming) {
                node.name = `${offer}_${buyer}_${designerColor}_${number}_${node.width.toFixed(
                    0
                )}x${node.height.toFixed(0)}`
            } else {
                // @ts-ignore
                node.name = `${findIdentificationLayer(
                    node,
                    offer
                )}_${buyer}_${designerColor}_${number}_${node.width.toFixed(0)}x${node.height.toFixed(0)}`.replace(
                    /\s/g,
                    ''
                )
            }
